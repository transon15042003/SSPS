  ...
  const filteredRows = data.filter((value) => {
    return (
      (selectedPrinter === "default" ||
        value.printer_name === selectedPrinter) &&
      (selectedTime.timeStart === null ||
        value.time_start >= selectedTime.timeStart) &&
      (selectedTime.timeEnd === null ||
        value.time_end <= selectedTime.timeEnd) &&
      (!user.isSPSO ||
        selectedCustomer === "" ||
        value.customer_name
          .toLowerCase()
    ...
    <>
      <div className="col-12 my-3">
        <FilterBar
          ...
          handleChoosePrinter={handleChoosePrinter}
