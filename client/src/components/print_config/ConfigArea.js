...
function ConfigArea({ num_pages, set_pages_state, support_function }) {
  ...
  useEffect(() => {
    const newData = originalData.filter(
      (item) => Number(item.loc_campus) === campusChange
    );
    setData(newData);
    const firstData = newData[0];
    setRoomValue(
      roomValue === "" ? "" : firstData.loc_building + "-" + firstData.loc_room
    );
  }, [campusChange, originalData, roomValue]);
  ...
  return (
    <>
        ...
        <select
          ...
          onChange={(event) => {
            ...
            const filteredData = data.filter(
              (item) => item.printer_id === input
            )[0];
            setRoomValue(
              filteredData.loc_building + "-" + filteredData.loc_room
            );
          }}
        >
          <option value="" disabled selected hidden>
      ...
      <div className="row p-2">
        <div className="col">Số trang</div>
        <select
          className="col form-select"
          id="pages-select"
          onChange={() => customSelection("pages-select")}
        >
          <option value="" disabled selected hidden>
            Chọn trang in
          </option>
      ...
      <div className={`row p-2 ${validation ? "d-none" : ""}`}>
        ...
        <div class="col alert alert-danger">
          <span>
            <GoAlertFill style={{ width: "0.9em", height: "0.9em" }} />
          </span>
          <span>{" " + alertContent}</span>
        </div>
      </div>
      <div className="row p-2">
        <div className="col">Mặt in</div>
      ...
      <div className="row p-2">
        <div className="col">Khổ giấy</div>
      ...
      <div className="row p-2">
        <div className="col">Hướng in</div>
      ...
      <div className="row p-2">
        <div className="col">Số trang/1 giấy in</div>
        <select
          className="col form-select"
          id="pages_per_sheet-select"
          onChange={() => customSelection("pages_per_sheet-select")}
        >
          <option value="" disabled selected hidden>
            Số trang/1 giấy in
          </option>
      ...
      <div className="row p-2">
        <div className="col">Tỷ lệ</div>
        <select
          className="col form-select"
          id="scale-select"
          onChange={() => customSelection("scale-select")}
        >
          <option value="" disabled selected hidden>
            Tỷ lệ
          </option>
      ...
      <div className="d-flex justify-content-around px-2 py-4">
        ...
        <Link className="btn btn-danger fw-medium" to="/print">
          Quay lại
        </Link>
      </div>
    </>
