...
function PrintReportDetail() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_SERVER_URL}/report/getReport?year=${year}&month=${month}`
      )
      .then((response) => {
  ...
  }, [cookies, month, navigate, removeCookie, token, year]);
      ...
      >
        ...
        <div className="col-12 mb-2">
          <div className="row justify-content-center">
            {data.length > 0 && (
              <button
                className="col-1 btn m-2 btn-primary"
                onClick={handlePrint}
              >
                <IoPrintOutline style={{ width: "25px", height: "25px" }} />
              </button>
            )}
            <Link className="col-2 btn btn-danger m-2" to="/report">
              Quay lại
            </Link>
          </div>
        </div>
