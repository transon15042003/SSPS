...
function PrintConfirm() {
  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      ...
      else if (fetchedConfig.pages === "Odd")
        temp = Math.floor((fetchedNumPages + 1) / 2);
      else if (fetchedConfig.pages === "Even")
        temp = Math.floor(fetchedNumPages / 2);
      else {
      ...
      setCampus(
        fetchedConfig.campus === 1 ? "Cơ sở Lý Thường Kiệt" : "Cơ sở Dĩ An"
      );
        ...
        if (response.data.status === "Success") {
          try {
            ...
            if (updateResponse.data.status === "Success") {
              ...
              console.error(
                "Error updating balance. Unexpected response:",
                updateResponse.data
              );
            }
          } catch (updateError) {
          ...
          console.error(
            "Error confirming print. Unexpected response:",
            response.data
          );
        }
      } catch (postError) {
    ...
    >
      ...
      >
        ...
        <div className="d-flex justify-content-center">
          ...
          <Link className="col-3 btn btn-danger fw-medium mx-2" to="/print">
            Hủy
          </Link>
        </div>
      </div>
